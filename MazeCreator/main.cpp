	...
	virtual bool OnUserUpdate(float fElapsedTime){
		...
		if (m_nVisitedCells < m_nMazeWidth * m_nMazeHeight){
			...
			if (!neighbours.empty()){
				...
				// No available neighbours so backtracking
				m_stack.pop();
			}
		}
		...
		for (int x = 0; x < m_nMazeWidth; x++){
			for (int y = 0; y < m_nMazeHeight; y++){
				// Each cell is inflated by m_nPathWidth, so filling it in
				for (int py = 0; py < m_nPathWidth; py++)
					for (int px = 0; px < m_nPathWidth; px++){
						if (m_maze[y * m_nMazeWidth + x] & CELL_VISITED)
							...
							Draw(x * (m_nPathWidth + 1) + px, y * (m_nPathWidth + 1) + py, PIXEL_SOLID, FG_DARK_GREY); // Draw Cell
					}

				// Drawing passageways betn cells
